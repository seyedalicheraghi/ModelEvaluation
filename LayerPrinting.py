...
if __name__ == "__main__":
    ...
    parser.add_argument("-y", "--Yaml",
                        help="Path to the Yaml file which includes supported operators!")
    parser.add_argument("-m", "--ONNXModel",
                        help="Name of the ONNX model to evaluate!")
    args = parser.parse_args()
    if not args.Yaml:
        raise Exception("Please select the Yaml file including supported operators!")
    supportedOperators = []
    yaml_file = str(args.Yaml)
    with open(yaml_file, "r") as stream:
        try:
            supportedOperators = yaml.safe_load(stream)
        except yaml.YAMLError as exc:
            print(exc)
    if not args.ONNXModel:
        raise Exception("Please select the ONNX model to evaluate!")

    onnx_directory = args.ONNXModel
    print("*" * 100)
